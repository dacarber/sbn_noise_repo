...
if not os.path.exists(directory):
    os.mkdir(directory)
files =uproot.open(f"/Users/danielcarber/Documents/SBND/Noise_Analysis/data/Run1_data/noise_output_fft_run_{Run_num}.root")
files[files.keys()[0]].keys()

raw_rms = files[files.keys()[0]][f'{noise}_rms'].array().to_list()
Noise_df = {'Channel_id':[],'Raw_rms':[],'wire_plane':[]}

bad_wires = [4800,4801,4802,4803,4804,4805,10438,10439,10440,10441,10442,104443]
bad_count = 0
for r in range(11276):
    if r in bad_wires:
        print("Bad",r)
        bad_count +=1
        continue
    ...
    if raw_rms[0] == 0 or np.isnan(raw_rms[r]):
        Noise_df['Raw_rms'].append(None)
    else:
        Noise_df['Raw_rms'].append(raw_rms[r])
    if r <1984:
...
fig.add_trace(go.Scatter(x=Noise_df['Channel_id'][mask],y = Noise_df['Raw_rms'][mask],marker_color = 'darkgreen'),row = 1, col = 2)
mask = Noise_df['wire_plane'] == 'UA'
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number
fig.add_trace(go.Scatter(x=Noise_df['Channel_id'][mask],y = Noise_df['Raw_rms'][mask],marker_color = 'darkgreen'),row = 1, col = 1)
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number

mask = Noise_df['wire_plane'] == 'VB'
fig.add_trace(go.Scatter(x=Noise_df['Channel_id'][mask],y = Noise_df['Raw_rms'][mask],marker_color = 'darkorange'),row = 2, col = 2)
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number
mask = Noise_df['wire_plane'] == 'VA'
fig.add_trace(go.Scatter(x=Noise_df['Channel_id'][mask],y = Noise_df['Raw_rms'][mask],marker_color = 'darkorange'),row = 2, col = 1)

#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number
mask = Noise_df['wire_plane'] == 'YB'
fig.add_trace(go.Scatter(x=Noise_df['Channel_id'][mask],y = Noise_df['Raw_rms'][mask],marker_color = 'purple'),row = 3, col = 2)
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1664)) for local channel number
mask = Noise_df['wire_plane'] == 'YA'
fig.add_trace(go.Scatter(x=Noise_df['Channel_id'][mask],y = Noise_df['Raw_rms'][mask],marker_color = 'purple'),row = 3, col = 1)
fig.add_vrect(
    #x0="1248", x1="1280",
    x0="3232", x1="3264",

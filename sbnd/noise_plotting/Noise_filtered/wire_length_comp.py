...
def binning(spectra,bin_size=10, mode = "mean"):
    max_length = np.max(spectra['r'])
    min_length = np.min(spectra['r'])
    smooth_spectra = []
    length = []
    for i in range(0,int(max_length),bin_size):
        mask = (spectra['r']>=i) & (spectra['r']<i+bin_size)
        smooth_spectra.append(np.mean(spectra['Raw_rms'][mask]))
        length.append(i+bin_size/2)
    return smooth_spectra,length
...
files_sim = uproot.open(f"/Users/danielcarber/Documents/SBND/Noise_Analysis/data/noise_output_runsimNF.root")
...
raw_rms_sim = files_sim['tpc_noise;5'][f'{noise}_rms'].array().to_list()
Noise_df_sim = {'Channel_id':[],'Raw_rms':[],'wire_plane':[]}
bad_count = 0
for r,rms in enumerate(raw_rms_sim): 
    if r in bad_wires:
        print("Bad",r)
        bad_count +=1
        continue
    if r == 0:
        continue
        #print(r-bad_count,rms)
    Noise_df_sim['Channel_id'].append(r-bad_count)
    print(rms)
    Noise_df_sim['Raw_rms'].append(rms)
    if r <1984:
        Noise_df_sim['wire_plane'].append('UB')
    elif r<3968:
        Noise_df_sim['wire_plane'].append('VB')
    elif r<5632:
        Noise_df_sim['wire_plane'].append('YB')
    elif r<7616:
        Noise_df_sim['wire_plane'].append('UA')
    elif r<9600:
        Noise_df_sim['wire_plane'].append('VA')
    else:
        Noise_df_sim['wire_plane'].append('YA')
Noise_df_sim = pd.DataFrame(Noise_df_sim)
...
Noise_df_sim =Noise_df_sim.merge(wire_df, on='Channel_id')
print(Noise_df)
...
#fig.add_trace(go.Scatter(x=Noise_df['r'][Noise_df['plane']==0][Noise_df['Raw_rms']>1.1],y = Noise_df['Raw_rms'][Noise_df['plane']==0][Noise_df['Raw_rms']>1.1]-Noise_df_sim['Raw_rms'][Noise_df_sim['plane']==0][Noise_df_sim['Raw_rms']>1.1],marker_color = 'DarkGreen',mode='markers',opacity=.8,text=Noise_df['Channel_id'][Noise_df['plane']==0][Noise_df['Raw_rms']>1.1],name='1<sup>st</sup> Induction'),row = 1, col = 1)
#fig.add_trace(go.Scatter(x=Noise_df['r'][Noise_df['plane']==1][Noise_df['Raw_rms']>1.1],y = Noise_df['Raw_rms'][Noise_df['plane']==1][Noise_df['Raw_rms']>1.1]-Noise_df_sim['Raw_rms'][Noise_df_sim['plane']==1][Noise_df_sim['Raw_rms']>1.1],marker_color = 'darkorange',mode='markers',opacity=.8,text=Noise_df['Channel_id'][Noise_df['plane']==1][Noise_df['Raw_rms']>1.1],name='2<sup>nd</sup> Induction'),row = 1, col = 1)
#fig.add_trace(go.Scatter(x=Noise_df['r'][Noise_df['plane']==2][Noise_df['Raw_rms']>1.4],y = Noise_df['Raw_rms'][Noise_df['plane']==2][Noise_df['Raw_rms']>1.4]-Noise_df_sim['Raw_rms'][Noise_df_sim['plane']==2][Noise_df_sim['Raw_rms']>1.4],marker_color = 'Purple',mode='markers',opacity=.8,text=Noise_df['Channel_id'][Noise_df['plane']==2][Noise_df['Raw_rms']>1.4],name='Collection'),row = 1, col = 1)
U_smooth,U_length = binning(Noise_df[Noise_df['plane']==0][Noise_df['Raw_rms']>1.1])
U_smooth_sim,U_length_sim = binning(Noise_df_sim[Noise_df_sim['plane']==0][Noise_df_sim['Raw_rms']>1.1])

V_smooth,V_length = binning(Noise_df[Noise_df['plane']==1][Noise_df['Raw_rms']>1.1])
V_smooth_sim,V_length_sim = binning(Noise_df_sim[Noise_df_sim['plane']==1][Noise_df_sim['Raw_rms']>1.1])

Y_smooth,Y_length = binning(Noise_df[Noise_df['plane']==2][Noise_df['Raw_rms']>1.4])
Y_smooth_sim,Y_length_sim = binning(Noise_df_sim[Noise_df_sim['plane']==2][Noise_df_sim['Raw_rms']>1.4])
print(len(Y_smooth),len(Y_smooth_sim))
fig.add_trace(go.Scatter(x=U_length,y =np.subtract(U_smooth,U_smooth_sim),marker_color = 'DarkGreen',mode='markers',opacity=.8,text=Noise_df['Channel_id'][Noise_df['plane']==0][Noise_df['Raw_rms']>1.1],name='1<sup>st</sup> Induction'),row = 1, col = 1)
fig.add_trace(go.Scatter(x=V_length,y = np.subtract(V_smooth,V_smooth_sim),marker_color = 'darkorange',mode='markers',opacity=.8,text=Noise_df['Channel_id'][Noise_df['plane']==1][Noise_df['Raw_rms']>1.1],name='2<sup>nd</sup> Induction'),row = 1, col = 1)
fig.add_trace(go.Scatter(x=Y_length,y = np.subtract(Y_smooth,Y_smooth_sim),marker_color = 'Purple',mode='markers',opacity=.8,text=Noise_df['Channel_id'][Noise_df['plane']==2][Noise_df['Raw_rms']>1.4],name='Collection'),row = 1, col = 1)

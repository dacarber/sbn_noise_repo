###################################################
#### Made by Dan Carber on 03/21/2025          ####               
...
for f in range(len(files)):
    ...
    df_orig[f'total'] = [0]*(int(event_length/2))
...
#print(df['total'])
    ...
    for channel in tqdm(range(0,11264,1)):
        if df_orig[f'{channel}'] == 'skip':
            ...
            df_orig[f'total'] =[df_orig[f'total'][j] + df_orig[f'{channel}'][j] for j in range(len(df_orig[f'total']))]
...
freq = list(range(len(df_orig['100'])))
...
fig = make_subplots(rows=1,cols =1,subplot_titles = (f"<span style='font-size: 26px;'>FFT Spectrum</span>",))
#fig_u.add_trace(go.Scatter(x=freq,y = np.divide(df[f'total'],11264-skipped_orig),marker_color = color[0],opacity = 1/(f+1),name = f"<span style='font-size: 16px;'>First Induction NF</span>"),row = 1, col = 1)
print(freq)
fig.add_trace(go.Scatter(x=freq,y = np.divide(df_orig[f'total'],11264-skipped_orig),marker_color = color[3],opacity = 1/(f+1),name = f"<span style='font-size: 16px;'>Run {Run_num}</span>"),row = 1, col = 1)
fig.add_annotation(dict(font = dict(size = 15,color="Black",),xshift= 600,yshift=340,text = f"<b>SBND<br>Preliminary Data</b>",showarrow = False),row = 1, col =1)
fig.update_xaxes(title_text = "<span style='font-size: 20px;'>Frequency [MHz]</span>",range = (0,1),row = 1, col = 1)
fig.update_yaxes(title_text = "<span style='font-size: 20px;'>Magnitude [mV/0.59 kHz]</span>",row = 1, col = 1)

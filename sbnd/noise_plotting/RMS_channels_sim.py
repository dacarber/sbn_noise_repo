...
sim = uproot.open(f"/Users/danielcarber/Documents/SBND/Noise_Analysis/data/noise_output_runsim.root")
...
raw_rms = sim['tpc_noise;1'][f'{noise}_rms'].array().to_list()
Noise_df = {'Channel_id':[],'Raw_rms':[],'wire_plane':[]}
for r,rms in enumerate(raw_rms):
    Noise_df['Channel_id'].append(r)
    if rms ==0:
...
#median = np.median(Noise_df['Raw_rms'][mask])
#mean = np.mean(Noise_df['Raw_rms'][mask])
fig.add_trace(go.Scatter(x=list(range(0,1984)),y = Noise_df['Raw_rms'][mask],marker_color = 'black'),row = 1, col = 2)
mask = Noise_df['wire_plane'] == 'UA'
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number
fig.add_trace(go.Scatter(x=list(range(0,1984)),y = Noise_df['Raw_rms'][mask],marker_color = 'black'),row = 1, col = 1)
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number

mask = Noise_df['wire_plane'] == 'VB'
fig.add_trace(go.Scatter(x=list(range(0,1984)),y = Noise_df['Raw_rms'][mask],marker_color = 'black'),row = 2, col = 2)
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number
mask = Noise_df['wire_plane'] == 'VA'
fig.add_trace(go.Scatter(x=list(range(0,1984)),y = Noise_df['Raw_rms'][mask],marker_color = 'black'),row = 2, col = 1)

#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1984)) for local channel number
mask = Noise_df['wire_plane'] == 'YB'
fig.add_trace(go.Scatter(x=list(range(0,1664)),y = Noise_df['Raw_rms'][mask],marker_color = 'black'),row = 3, col = 2)
#Noise_df['Channel_id'][mask] for LArSoft Channels, list(range(0,1664)) for local channel number
mask = Noise_df['wire_plane'] == 'YA'
fig.add_trace(go.Scatter(x=list(range(0,1664)),y = Noise_df['Raw_rms'][mask],marker_color = 'black'),row = 3, col = 1)

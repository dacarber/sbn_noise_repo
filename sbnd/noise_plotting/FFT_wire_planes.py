...
files.append(uproot.open(f"/Users/danielcarber/Documents/SBND/Noise Analysis/fft_output_6000_run11541.root"))
files.append(uproot.open(f"/Users/danielcarber/Documents/SBND/Noise Analysis/fft_output_8000_run11541.root"))
files.append(uproot.open(f"/Users/danielcarber/Documents/SBND/Noise Analysis/fft_output_10000_run11541.root"))
...
fig.add_trace(go.Scatter(x=freq,y = np.divide(df['total_1'],1000),marker_color = 'green'),row = 2, col = 1)
fig.add_trace(go.Scatter(x=freq,y = np.divide(df['total_2'],1000),marker_color = 'blue'),row = 3, col = 1)

fig.update_layout(xaxis = dict(range = [1*0,.996*1]),xaxis2 = dict(range = [1*0,.996*1]),xaxis3 = dict(range = [1*0,.996*1]))
fig.update_xaxes(title_text = "Frequency [MHz]",row = 3, col = 1)
fig.update_layout(xaxis = dict(tickmode = 'linear',dtick = .02),xaxis2 = dict(tickmode = 'linear',dtick = .02),xaxis3 = dict(tickmode = 'linear',dtick = .02))

...
files.append(uproot.open(f"/Users/danielcarber/Documents/SBND/Noise_Analysis/data/noise_output_run17470_test.root"))
files.append(uproot.open(f"/Users/danielcarber/Documents/SBND/Noise_Analysis/data/noise_output_sim_test.root"))


event_length = 3427#3427 #3418
...
for f in range(len(files)):
    ...
    df[f'total_U_{f}'] = [0]*(int(event_length/2)+2)
    df[f'total_V_{f}'] = [0]*(int(event_length/2)+2)
    df[f'total_Y_{f}'] = [0]*(int(event_length/2)+2)
...
#print(df['total'])
    ...
    for i in tqdm(range(11264)):
        ...
        if raw_rms[(int(event_length/2)+1)*(i+1)+i]==0:
            df[f'{i}'] = 'skip'
            continue
        df[f'{i}'] =list((raw_rms[i*(int(event_length/2)+2):(i+1)*(int(event_length/2)+2)]/raw_rms[(int(event_length/2)+1)*(i+1)+i])*(1800/4095))
    skipped_U = 0
    skipped_Y = 0
    skipped_V = 0
    ...
    fig.add_trace(go.Scatter(x=freq,y = np.divide(df[f'total_U_{f}'],1984*2-skipped_U),marker_color = color[i],opacity = 1/(f+1),name = f"<span style='font-size: 16px;'>First Induction</span>"),row = 1, col = 1)
    i+=1
    ...
    #fig.add_trace(go.Scatter(x=freq,y = np.divide(df[f'total_Y_{f}'],1664*2-skipped_Y),marker_color = color[2],opacity = 1/(f+1),name = f"<span style='font-size: 16px;'>Collection</span>"),row = 1, col = 1)

fig.update_xaxes(title_text = "<span style='font-size: 20px;'>Frequency [MHz]</span>",row = 1, col = 1)
fig.update_yaxes(title_text = "<span style='font-size: 20px;'>Magnitude [mV/0.59 kHz]</span>",range = (0,93),row = 1, col = 1)

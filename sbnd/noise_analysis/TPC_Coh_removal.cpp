	...
	while (Events.Next())
	{
		...
		for(int ki=0; ki<11264;ki++){
			...
			if (myADC[index].NADC() != 3415 && (ki+1)%group_size != 0){
				...
				transform(RMS_wave_total[channel/(group_size-1)].begin(),RMS_wave_total[channel/(group_size-1)].end(),coherent_waveform.begin(),RMS_wave_total[channel/(group_size-1)].begin(),plus<float>());

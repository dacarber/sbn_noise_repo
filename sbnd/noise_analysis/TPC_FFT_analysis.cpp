...
vector<float> FFT(vector<double> noise_channel){
	//vector<float> FFT;
	...
	double fftReal=0;
        double fftImag=0;
	vector<float> fftMag(vec_size / 2 + 1);
...
{	
	...
	vector<vector<float>> FFT_total(11264,vector<float>(3415/2+1,0));
	...
	//	cout<<myPedestal.GetSize()<<endl;
		...
		for(int ki=0; ki<myADC.GetSize();ki++){
			...
			vector<float> channel_fft = FFT(x); 
			//cout<<"FFT is calculated"<<endl;
			transform(FFT_total[ki].begin(),FFT_total[ki].end(),channel_fft.begin(),FFT_total[ki].begin(),plus<float>());
	...
	for(int ch = 0; ch<1000; ch++){
		transform(FFT_total[ch].begin(),FFT_total[ch].end(),FFT_total[ch].begin(),[evt](float &c){ return c/evt; });
